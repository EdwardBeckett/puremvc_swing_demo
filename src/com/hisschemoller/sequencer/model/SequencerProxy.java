...
public class SequencerProxy extends Proxy implements ISequenceable {
	...
	public void onNoteStart( PatternVO patternVO, MidiEvent midiEvent ) {
		...
		try {
			if( _isMidiEnabled ) {
				...
				shortMessage.setMessage( ShortMessage.NOTE_ON, message.getChannel(), message.getData1(),
					message.getData2() );
				sendNotification( SeqNotifications.SEND_MIDI_MESSAGE, shortMessage );
			}
		} catch( InvalidMidiDataException exception ) {
			System.out.println(
				"SequencerProxy.onNoteStart() exception - Invalid MIDI data: " + exception.getMessage() );
		}
		...
		PatternSequenceNote note = new PatternSequenceNote( (int) ( midiEvent.getTick() / patternVO.stepLength ),
			ShortMessage.NOTE_ON, patternVO.id );
	...
	public void onNoteEnd( PatternVO patternVO, MidiEvent midiEvent ) {
		...
		try {
			if( _isMidiEnabled ) {
				ShortMessage shortMessage = new ShortMessage();
				shortMessage.setMessage( ShortMessage.NOTE_OFF, message.getChannel(), message.getData1(),
					message.getData2() );
				sendNotification( SeqNotifications.SEND_MIDI_MESSAGE, shortMessage );
			}
		} catch( InvalidMidiDataException exception ) {
		...
		/** Send view update notification. */
		PatternSequenceNote note = new PatternSequenceNote( (int) ( midiEvent.getTick() / patternVO.stepLength ),
			ShortMessage.NOTE_OFF, patternVO.id );
		sendNotification( SeqNotifications.PATTERN_SEQUENCE_UPDATED, note );
	}
