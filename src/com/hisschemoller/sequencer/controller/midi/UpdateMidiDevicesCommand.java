	...
	public final void execute( final INotification notification ) {
		...
		for(int i = 0; i < n; i++) {
			try {
				...
				System.out.println(
					"UpdateMidiDevicesCommand - MIDI device found, name: " + midiDeviceInfo[i].getName() + ", description: " + midiDeviceInfo[i]
						.getDescription() + " (Max. MIDI IN: " + midiDevice.getMaxTransmitters() + ", max. MIDI OUT: " + midiDevice
						.getMaxReceivers() + ")" );
			} catch( MidiUnavailableException exception ) {
				System.out.println(
					"UpdateMidiDevicesCommand exception - MIDI device unavailable: " + exception.getMessage() );
			}
		}
