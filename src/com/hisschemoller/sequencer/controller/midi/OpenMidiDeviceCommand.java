...
public class OpenMidiDeviceCommand extends SimpleCommand {
	...
	public final void execute( final INotification notification ) {
		...
		try {
			...
			System.out.println(
				"OpenMidiDeviceCommand - MIDI device: " + midiDeviceInfo.getDescription() + ", notification: " + notification
					.getName() );
			...
			if( notification.getName() == SeqNotifications.OPEN_MIDI_IN_DEVICE ) {
				...
				if( notification.getName() == SeqNotifications.OPEN_MIDI_OUT_DEVICE ) {
			...
			if( !midiDevice.isOpen() ) {
				try {
					...
					System.out.println(
						"OpenMidiDeviceCommand exception - MIDI device can't be opened: " + exception.getMessage() );
				}
			}
		} catch( MidiUnavailableException exception ) {
			System.out.println(
				"OpenMidiDeviceCommand exception - MIDI device unavailable: " + exception.getMessage() );
		}
	}
