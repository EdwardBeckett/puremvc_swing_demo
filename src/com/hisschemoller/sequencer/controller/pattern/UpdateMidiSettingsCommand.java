...
public class UpdateMidiSettingsCommand extends SimpleCommand {
	...
	public final void execute( final INotification notification ) {
		...
		for(int i = 0; i < patternVO.fills; i++) {
			...
			try {
				message.setMessage( ShortMessage.NOTE_ON, settingsVO.midiChannel, settingsVO.midiPitch,
					settingsVO.midiVelocity );
			} catch( InvalidMidiDataException exception ) {
				System.out.println(
					"UpdateMidiSettingsCommand.execute() InvalidMidiDataException: " + exception.getMessage() );
			}
		}
