...
public class PatternEditorMediator extends Mediator implements IViewEventListener {
	...
	public void handleNotification( INotification note ) {
		if( note.getName() == SeqNotifications.REDRAW ) {
			...
			if( note.getName() == SeqNotifications.PATTERN_SEQUENCE_UPDATED ) {
				...
				if( note.getName() == SeqNotifications.PATTERN_SETTINGS_UPDATED ) {
					...
					if( note.getName() == SeqNotifications.PATTERN_LOCATION_UPDATED ) {
						getView().updatePattern( (PatternVO) note.getBody(), Pattern.Operation.LOCATION );
					} else {
						if( note.getName() == SeqNotifications.SELECT_PATTERN ) {
							getView().selectPattern( (PatternVO) note.getBody() );
						} else {
							if( note.getName() == SeqNotifications.PATTERN_CREATED ) {
								getView().addPattern( (PatternVO) note.getBody(),
									Boolean.parseBoolean( note.getType() ) );
							} else {
								if( note.getName() == SeqNotifications.PATTERN_DELETED ) {
									getView().deletePattern( (PatternVO) note.getBody() );
								} else {
									if( note.getName() == SeqNotifications.PATTERN_MUTED ) {
										getView().updatePattern( (PatternVO) note.getBody(), Pattern.Operation.MUTE );
									} else {
										if( note.getName() == SeqNotifications.PATTERN_SOLOED ) {
											getView().updateAllPatterns( (PatternVO) note.getBody(),
												Pattern.Operation.SOLO );
										} else {
											if( note.getName() == SeqNotifications.PATTERN_NAME_UPDATED ) {
												getView().updatePattern( (PatternVO) note.getBody(),
													Pattern.Operation.NAME );
											} else {
												if( note.getName() == SeqNotifications.PLAYBACK_CHANGED ) {
													getView().updatePlayback(
														(SequencerEnums.Playback) note.getBody() );
												}
											}
										}
									}
