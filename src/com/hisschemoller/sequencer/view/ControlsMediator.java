...
public class ControlsMediator extends Mediator implements IViewEventListener {
	...
	public void handleNotification( INotification note ) {
		...
		if( name == SeqNotifications.TEMPO_UPDATED ) {
			...
			if( name == SeqNotifications.PLAYBACK_CHANGED ) {
				...
				if( note.getName() == SeqNotifications.RESOLUTION_UPDATED ) {
					...
					if( name == SeqNotifications.MIDI_DEVICES_UPDATED ) {
						...
						if( name == SeqNotifications.MIDI_OUT_DEVICE_OPENED ) {
							getView().setMidiOutDevice( (MidiDevice.Info) note.getBody() );
						} else {
							if( name == SeqNotifications.MIDI_OUT_DEVICE_ENABLED ) {
								getView().updateMidiOutEnabled( (Boolean) note.getBody() );
							} else {
								if( name == SeqNotifications.OSC_DEVICE_ENABLED ) {
									getView().updateOscEnabled( (Boolean) note.getBody() );
								} else {
									if( name == SeqNotifications.OSC_PORT_UPDATED ) {
	...
	public void viewEventHandler( ViewEvent event ) {
		switch( event.getID() ) {
			...
			case ViewEvent.MIDI_CHECKBOX_SELECT:
				sendNotification( SeqNotifications.ENABLE_MIDI_OUT_DEVICE,
			...
			case ViewEvent.OSC_PORT_CHANGE:
				sendNotification( SeqNotifications.UPDATE_OSC_PORT,
