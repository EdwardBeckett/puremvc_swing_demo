...
public class PatternSettingsMediator extends Mediator implements IViewEventListener {
	...
	public void handleNotification( INotification note ) {
		if( note.getName() == SeqNotifications.PATTERN_SETTINGS_UPDATED ) {
			...
			if( note.getName() == SeqNotifications.MIDI_SETTINGS_UPDATED ) {
				getView().updateSettings( (PatternVO) note.getBody() );
			} else {
				if( note.getName() == SeqNotifications.SELECT_PATTERN ) {
					getView().updatePattern( (PatternVO) note.getBody() );
				} else {
					if( note.getName() == SeqNotifications.PATTERN_QUANTIZATION_UPDATED ) {
						getView().updateSettings( (PatternVO) note.getBody() );
					} else {
						if( note.getName() == SeqNotifications.PATTERN_SOLOED ) {
							...
							if( note.getName() == SeqNotifications.PATTERN_MUTED ) {
								...
								if( note.getName() == SeqNotifications.PATTERN_NAME_UPDATED ) {
									...
									if( note.getName() == SeqNotifications.RESOLUTION_UPDATED ) {
										getView().updateResolution( (Integer) note.getBody() );
									} else {
										if( note.getName() == SeqNotifications.OSC_SETTINGS_UPDATED ) {
											getView().updateSettings( (PatternVO) note.getBody() );
										}
									}
								}
